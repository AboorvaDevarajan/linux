...
bool pmem_should_map_pages(struct device *dev)
{
	...
	if (!IS_ENABLED(CONFIG_ZONE_DEVICE)) {
		printk(KERN_INFO "%s: EXIT: false (ZONE_DEVICE disabled) pid=%d\n", __func__, current->pid);
		return false;
	}
	...
	if (is_nd_pfn(dev) || is_nd_btt(dev)) {
		printk(KERN_INFO "%s: EXIT: false (is_nd_pfn or is_nd_btt) pid=%d\n", __func__, current->pid);
		return false;
	}
...
static ssize_t __alt_name_store(struct device *dev, const char *buf,
		const size_t len)
{
	printk(KERN_INFO "%s: ENTRY: dev=%p, buf=%p, len=%zu pid=%d\n", __func__, dev, buf, len, current->pid);
	...
	if (!input) {
		printk(KERN_INFO "%s: EXIT: -ENOMEM (input) pid=%d\n", __func__, current->pid);
		return -ENOMEM;
	}
		...
		struct device *dev)
{
	...
	if (dev->driver || to_ndns(dev)->claim) {
		printk(KERN_INFO "%s: EXIT: 0 (driver/claim) pid=%d\n", __func__, current->pid);
		return 0;
	}
	...
	if (is_namespace_pmem(dev)) {
		...
		if (size == 0 && nspm->uuid)
			...
			printk(KERN_INFO "%s: EXIT: 0 (no uuid) pid=%d\n", __func__, current->pid);
			return 0;
		}

		int rc = nd_pmem_namespace_label_update(nd_region, nspm, size);
		...
		resource_size_t n)
{
	...
	while (n) {
		...
		res = last;
		if (!res) {
			printk(KERN_INFO "%s: EXIT: 0 (no res) pid=%d\n", __func__, current->pid);
			return 0;
		}
...
struct nd_namespace_common *nvdimm_namespace_common_probe(struct device *dev)
{
	...
	int rc = 0;
	if (!ndns) {
		printk(KERN_INFO "%s: EXIT: -ENODEV (no ndns) pid=%d\n", __func__, current->pid);
		return ERR_PTR(-ENODEV);
	}
	if (!ndns->claim_class) {
		printk(KERN_INFO "%s: EXIT: -EINVAL (no claim_class) pid=%d\n", __func__, current->pid);
		return ERR_PTR(-EINVAL);
	}
	rc = ndns->claim_class->probe(dev);
	printk(KERN_INFO "%s: EXIT: rc=%d pid=%d\n", __func__, rc, current->pid);
	return ndns;
}
					    ...
					    struct nd_namespace_label *nd_label)
{
	...
	if (cookie == 0) {
		...
		printk(KERN_INFO "%s: EXIT: ERR_PTR(-ENXIO) (cookie==0) pid=%d\n", __func__, current->pid);
		return ERR_PTR(-ENXIO);
	}
	if (!nsl_validate_isetcookie(ndd, nd_label, cookie)) {
		dev_dbg(&nd_region->dev, "invalid cookie in label: %pUb\n", nsl_uuid_raw(ndd, nd_label));
		if (!nsl_validate_isetcookie(ndd, nd_label, altcookie)) {
			printk(KERN_INFO "%s: EXIT: ERR_PTR(-EAGAIN) (altcookie) pid=%d\n", __func__, current->pid);
	...
	if (!nspm) {
		printk(KERN_INFO "%s: EXIT: ERR_PTR(-ENOMEM) (kzalloc) pid=%d\n", __func__, current->pid);
...
static struct device *nd_namespace_pmem_create(struct nd_region *nd_region)
{
	...
	if (!is_memory(&nd_region->dev)) {
		printk(KERN_INFO "%s: EXIT: NULL (not memory) pid=%d\n", __func__, current->pid);
		return NULL;
	}
	nspm = kzalloc(sizeof(*nspm), GFP_KERNEL);
	if (!nspm) {
		printk(KERN_INFO "%s: EXIT: NULL (kzalloc) pid=%d\n", __func__, current->pid);
		return NULL;
	}
	dev = &nspm->nsio.common.dev;
	dev->type = &namespace_pmem_device_type;
	...
	if (nspm->id < 0) {
		kfree(nspm);
		printk(KERN_INFO "%s: EXIT: NULL (ida_alloc) pid=%d\n", __func__, current->pid);
		return NULL;
	}
...
void nd_region_create_ns_seed(struct nd_region *nd_region)
{
	...
	if (nd_region_to_nstype(nd_region) == ND_DEVICE_NAMESPACE_IO) {
		printk(KERN_INFO "%s: EXIT (IO type) pid=%d\n", __func__, current->pid);
		return;
	}
...
static struct device **scan_labels(struct nd_region *nd_region)
{
	...
	if (!devs) {
		printk(KERN_INFO "%s: EXIT: NULL (kcalloc) pid=%d\n", __func__, current->pid);
		return NULL;
	}
...
static struct device **create_namespaces(struct nd_region *nd_region)
{
	...
	int i;
	if (nd_region->ndr_mappings == 0) {
		printk(KERN_INFO "%s: EXIT: NULL (ndr_mappings==0) pid=%d\n", __func__, current->pid);
		return NULL;
	}
	for (i = 0; i < nd_region->ndr_mappings; i++) {
		nd_mapping = &nd_region->mapping[i];
...
int nd_region_register_namespaces(struct nd_region *nd_region, int *err)
{
	...
	*err = 0;
	...
	if (rc) {
		...
		printk(KERN_INFO "%s: EXIT: rc=%d (init_active_labels) pid=%d\n", __func__, rc, current->pid);
		return rc;
	}
