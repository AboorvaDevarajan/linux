...
struct nd_dax *to_nd_dax(struct device *dev)
{
	...
	printk(KERN_INFO "%s: EXIT: nd_dax=%p pid=%d\n", __func__, nd_dax, current->pid);
	return nd_dax;
}
...
static struct nd_dax *nd_dax_alloc(struct nd_region *nd_region)
{
	...
	if (!nd_dax) {
		printk(KERN_INFO "%s: EXIT: nd_dax allocation failed pid=%d\n", __func__, current->pid);
		return NULL;
	}
	...
	if (nd_pfn->id < 0) {
		kfree(nd_dax);
		printk(KERN_INFO "%s: EXIT: ida_alloc failed pid=%d\n", __func__, current->pid);
		return NULL;
	}
...
struct device *nd_dax_create(struct nd_region *nd_region)
{
	...
	if (!is_memory(&nd_region->dev)) {
		printk(KERN_INFO "%s: EXIT: not memory region pid=%d\n", __func__, current->pid);
		return NULL;
	}
...
int nd_dax_probe(struct device *dev, struct nd_namespace_common *ndns)
{
	...
	if (ndns->force_raw) {
		printk(KERN_INFO "%s: EXIT: force_raw set pid=%d\n", __func__, current->pid);
		return -ENODEV;
	}
	...
	case NVDIMM_CCLASS_DAX:
		break;
	default:
		printk(KERN_INFO "%s: EXIT: claim_class not supported pid=%d\n", __func__, current->pid);
		return -ENODEV;
	}
	...
	if (!dax_dev) {
		printk(KERN_INFO "%s: EXIT: dax_dev allocation failed pid=%d\n", __func__, current->pid);
		return -ENOMEM;
	}
