...
int nvdimm_check_config_data(struct device *dev)
{
	...
	if (!nvdimm->cmd_mask ||
		...
		if (test_bit(NDD_LABELING, &nvdimm->flags)) {
			printk(KERN_INFO "%s: EXIT: -ENXIO (labeling) pid=%d\n", __func__, current->pid);
			return -ENXIO;
		} else {
			printk(KERN_INFO "%s: EXIT: -ENOTTY (no cmd_mask) pid=%d\n", __func__, current->pid);
			return -ENOTTY;
		}
	}
...
static int validate_dimm(struct nvdimm_drvdata *ndd)
{
	...
	if (!ndd) {
		printk(KERN_INFO "%s: EXIT: -EINVAL (ndd is NULL) pid=%d\n", __func__, current->pid);
		return -EINVAL;
	}
...
int nvdimm_init_nsarea(struct nvdimm_drvdata *ndd)
{
	...
	if (rc) {
		printk(KERN_INFO "%s: EXIT: rc=%d (validate_dimm failed) pid=%d\n", __func__, rc, current->pid);
		return rc;
	}

	if (cmd->config_size) {
		printk(KERN_INFO "%s: EXIT: 0 (already valid) pid=%d\n", __func__, current->pid);
	...
	if (rc < 0) {
		printk(KERN_INFO "%s: EXIT: rc=%d (ndctl failed) pid=%d\n", __func__, rc, current->pid);
		return rc;
	}
	printk(KERN_INFO "%s: EXIT: cmd_rc=%d pid=%d\n", __func__, cmd_rc, current->pid);
	return cmd_rc;
}
...
{
	printk(KERN_INFO "%s: ENTRY: ndd=%p, buf=%p, offset=%zu, len=%zu pid=%d\n", __func__, ndd, buf, offset, len, current->pid);
	struct nvdimm_bus *nvdimm_bus = walk_to_nvdimm_bus(ndd->dev);
	...
	if (offset + len > ndd->nsarea.config_size) {
		printk(KERN_INFO "%s: EXIT: -ENXIO (out of bounds) pid=%d\n", __func__, current->pid);
		return -ENXIO;
	}
	...
	if (!cmd) {
		printk(KERN_INFO "%s: EXIT: -ENOMEM (kvzalloc failed) pid=%d\n", __func__, current->pid);
		return -ENOMEM;
	}
	...
	for (buf_offset = 0; len; len -= cmd->in_length, buf_offset += cmd->in_length) {
		...
		if (cmd_rc < 0) {
			rc = cmd_rc;
			printk(KERN_INFO "%s: EXIT: cmd_rc=%d (cmd_rc failed) pid=%d\n", __func__, cmd_rc, current->pid);
			break;
		}
...
int nvdimm_set_config_data(struct nvdimm_drvdata *ndd, size_t offset,
		void *buf, size_t len)
{
	printk(KERN_INFO "%s: ENTRY: ndd=%p, offset=%zu, buf=%p, len=%zu pid=%d\n", __func__, ndd, offset, buf, len, current->pid);
	size_t max_cmd_size, buf_offset;
...
struct nvdimm *to_nvdimm(struct device *dev)
{
	...
	printk(KERN_INFO "%s: EXIT: nvdimm=%p pid=%d\n", __func__, nvdimm, current->pid);
	return nvdimm;
}
...
{
	printk(KERN_INFO "%s: ENTRY: kref=%p pid=%d\n", __func__, kref, current->pid);
	...
	kfree(ndd);
	put_device(dev);
	printk(KERN_INFO "%s: EXIT: dev=%p, ndd=%p pid=%d\n", __func__, dev, ndd, current->pid);
}
...
{
	printk(KERN_INFO "%s: ENTRY: nvdimm=%p pid=%d\n", __func__, nvdimm, current->pid);
	const char *ret = dev_name(&nvdimm->dev);
	printk(KERN_INFO "%s: EXIT: ret=%s pid=%d\n", __func__, ret, current->pid);
	return ret;
}
...
void *nvdimm_provider_data(struct nvdimm *nvdimm)
{
	...
	printk(KERN_INFO "%s: EXIT: NULL (nvdimm is NULL) pid=%d\n", __func__, current->pid);
	return NULL;
}
...
static ssize_t commands_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (!nvdimm->cmd_mask) {
		...
		printk(KERN_INFO "%s: EXIT: ret=%zd (no cmd_mask) pid=%d\n", __func__, ret, current->pid);
		return ret;
	}
	...
	len += sprintf(buf + len, "\n");
	printk(KERN_INFO "%s: EXIT: len=%d pid=%d\n", __func__, len, current->pid);
	return len;
}
...
{
	printk(KERN_INFO "%s: ENTRY: ndd=%p, buf=%p pid=%d\n", __func__, ndd, buf, current->pid);
	struct device *dev;
	ssize_t rc;
	u32 nfree;

	if (!ndd) {
		printk(KERN_INFO "%s: EXIT: -ENXIO (ndd is NULL) pid=%d\n", __func__, current->pid);
		return -ENXIO;
	}
...
static ssize_t security_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (test_bit(NVDIMM_SECURITY_OVERWRITE, &nvdimm->sec.flags)) {
		...
		printk(KERN_INFO "%s: EXIT: ret=%zd (overwrite) pid=%d\n", __func__, ret, current->pid);
		return ret;
	}
	if (test_bit(NVDIMM_SECURITY_DISABLED, &nvdimm->sec.flags)) {
		ssize_t ret = sprintf(buf, "disabled\n");
		printk(KERN_INFO "%s: EXIT: ret=%zd (disabled) pid=%d\n", __func__, ret, current->pid);
		return ret;
	}
	if (test_bit(NVDIMM_SECURITY_UNLOCKED, &nvdimm->sec.flags)) {
		ssize_t ret = sprintf(buf, "unlocked\n");
		printk(KERN_INFO "%s: EXIT: ret=%zd (unlocked) pid=%d\n", __func__, ret, current->pid);
		return ret;
	}
	if (test_bit(NVDIMM_SECURITY_LOCKED, &nvdimm->sec.flags)) {
		ssize_t ret = sprintf(buf, "locked\n");
		printk(KERN_INFO "%s: EXIT: ret=%zd (locked) pid=%d\n", __func__, ret, current->pid);
		return ret;
	}
	printk(KERN_INFO "%s: EXIT: -ENOTTY (no state) pid=%d\n", __func__, current->pid);
	return -ENOTTY;
}
...
static umode_t nvdimm_visible(struct kobject *kobj, struct attribute *a, int n)
{
	...
	if (!nvdimm->sec.flags) {
		printk(KERN_INFO "%s: EXIT: 0 (no sec.flags) pid=%d\n", __func__, current->pid);
		return 0;
	}
	...
	if (a == &dev_attr_security.attr) {
		if (nvdimm->sec.ops->freeze || nvdimm->sec.ops->disable || nvdimm->sec.ops->change_key || nvdimm->sec.ops->erase || nvdimm->sec.ops->overwrite) {
			printk(KERN_INFO "%s: EXIT: mode=%o (security ops) pid=%d\n", __func__, a->mode, current->pid);
			return a->mode;
		}
		printk(KERN_INFO "%s: EXIT: 0444 (security read-only) pid=%d\n", __func__, current->pid);
		return 0444;
	}

	if (nvdimm->sec.ops->freeze) {
		printk(KERN_INFO "%s: EXIT: mode=%o (freeze) pid=%d\n", __func__, a->mode, current->pid);
		return a->mode;
	}
	printk(KERN_INFO "%s: EXIT: 0 (no freeze) pid=%d\n", __func__, current->pid);
	return 0;
}
...
static ssize_t result_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (!nvdimm->fw_ops) {
		printk(KERN_INFO "%s: EXIT: -EOPNOTSUPP (no fw_ops) pid=%d\n", __func__, current->pid);
		return -EOPNOTSUPP;
	}
	...
	case NVDIMM_FWA_RESULT_NONE: {
		...
		printk(KERN_INFO "%s: EXIT: ret=%zd (none) pid=%d\n", __func__, ret, current->pid);
		return ret;
	}
	case NVDIMM_FWA_RESULT_SUCCESS: {
		ssize_t ret = sprintf(buf, "success\n");
		printk(KERN_INFO "%s: EXIT: ret=%zd (success) pid=%d\n", __func__, ret, current->pid);
		return ret;
	}
	case NVDIMM_FWA_RESULT_FAIL: {
		ssize_t ret = sprintf(buf, "fail\n");
		printk(KERN_INFO "%s: EXIT: ret=%zd (fail) pid=%d\n", __func__, ret, current->pid);
		return ret;
	}
	case NVDIMM_FWA_RESULT_NOTSTAGED: {
		ssize_t ret = sprintf(buf, "not_staged\n");
		printk(KERN_INFO "%s: EXIT: ret=%zd (not_staged) pid=%d\n", __func__, ret, current->pid);
		return ret;
	}
	case NVDIMM_FWA_RESULT_NEEDRESET: {
		ssize_t ret = sprintf(buf, "need_reset\n");
		printk(KERN_INFO "%s: EXIT: ret=%zd (need_reset) pid=%d\n", __func__, ret, current->pid);
		return ret;
	}
	default:
		printk(KERN_INFO "%s: EXIT: -ENXIO (default) pid=%d\n", __func__, current->pid);
		return -ENXIO;
	}
}
...
static ssize_t activate_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	case NVDIMM_FWA_IDLE: {
		...
		printk(KERN_INFO "%s: EXIT: ret=%zd (idle) pid=%d\n", __func__, ret, current->pid);
		return ret;
	}
	case NVDIMM_FWA_BUSY: {
		ssize_t ret = sprintf(buf, "busy\n");
		printk(KERN_INFO "%s: EXIT: ret=%zd (busy) pid=%d\n", __func__, ret, current->pid);
		return ret;
	}
	case NVDIMM_FWA_ARMED: {
		ssize_t ret = sprintf(buf, "armed\n");
		printk(KERN_INFO "%s: EXIT: ret=%zd (armed) pid=%d\n", __func__, ret, current->pid);
		return ret;
	}
	default:
		printk(KERN_INFO "%s: EXIT: -ENXIO (default) pid=%d\n", __func__, current->pid);
...
static ssize_t activate_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t len)
{
	...
	if (sysfs_streq(buf, "arm"))
		...
		printk(KERN_INFO "%s: EXIT: -EINVAL (invalid arg) pid=%d\n", __func__, current->pid);
		return -EINVAL;
	}
	...
	if (rc < 0) {
		printk(KERN_INFO "%s: EXIT: rc=%d (fw_ops->arm failed) pid=%d\n", __func__, rc, current->pid);
		return rc;
	}
...
static umode_t nvdimm_firmware_visible(struct kobject *kobj, struct attribute *a, int n)
{
	...
	if (!nd_desc->fw_ops) {
		printk(KERN_INFO "%s: EXIT: 0 (no fw_ops) pid=%d\n", __func__, current->pid);
		return 0;
	}
	if (!nvdimm->fw_ops) {
		printk(KERN_INFO "%s: EXIT: 0 (no nvdimm fw_ops) pid=%d\n", __func__, current->pid);
		return 0;
	}
	...
	if (cap < NVDIMM_FWA_CAP_QUIESCE) {
		printk(KERN_INFO "%s: EXIT: 0 (cap < QUIESCE) pid=%d\n", __func__, current->pid);
		return 0;
	}
		...
		const struct nvdimm_fw_ops *fw_ops)
{
	...
	struct device *dev;
	printk(KERN_INFO "%s: ENTRY: nvdimm_bus=%p, provider_data=%p, groups=%p, flags=%lx, cmd_mask=%lx, num_flush=%d, flush_wpq=%p, dimm_id=%s, sec_ops=%p, fw_ops=%p pid=%d\n", __func__, nvdimm_bus, provider_data, groups, flags, cmd_mask, num_flush, flush_wpq, dimm_id, sec_ops, fw_ops, current->pid);
	if (!nvdimm) {
		printk(KERN_INFO "%s: EXIT: NULL (nvdimm allocation failed) pid=%d\n", __func__, current->pid);
		return NULL;
	}
	...
	if (nvdimm->id < 0) {
		kfree(nvdimm);
		printk(KERN_INFO "%s: EXIT: NULL (id allocation failed) pid=%d\n", __func__, current->pid);
		return NULL;
	}
...
int nvdimm_security_freeze(struct nvdimm *nvdimm)
{
	...
	if (!nvdimm->sec.ops || !nvdimm->sec.ops->freeze) {
		printk(KERN_INFO "%s: EXIT: -EOPNOTSUPP (no freeze op) pid=%d\n", __func__, current->pid);
		return -EOPNOTSUPP;
	}

	if (!nvdimm->sec.flags) {
		printk(KERN_INFO "%s: EXIT: -EIO (no sec.flags) pid=%d\n", __func__, current->pid);
		return -EIO;
	}

	if (test_bit(NDD_SECURITY_OVERWRITE, &nvdimm->flags)) {
		dev_warn(&nvdimm->dev, "Overwrite operation in progress.\n");
		printk(KERN_INFO "%s: EXIT: -EBUSY (overwrite in progress) pid=%d\n", __func__, current->pid);
		return -EBUSY;
	}
...
{
	printk(KERN_INFO "%s: ENTRY: ndd=%p, res=%p pid=%d\n", __func__, ndd, res, current->pid);
...
struct resource *nvdimm_allocate_dpa(struct nvdimm_drvdata *ndd,
		...
		resource_size_t n)
{
	printk(KERN_INFO "%s: ENTRY: ndd=%p, label_id=%p, start=%llu, n=%llu pid=%d\n", __func__, ndd, label_id, start, n, current->pid);
	char *name = kmemdup(label_id, sizeof(*label_id), GFP_KERNEL);
	struct resource *res;

	if (!name) {
		printk(KERN_INFO "%s: EXIT: NULL (kmemdup failed) pid=%d\n", __func__, current->pid);
		return NULL;
	}
	...
	if (!res) {
		kfree(name);
		printk(KERN_INFO "%s: EXIT: NULL (__request_region failed) pid=%d\n", __func__, current->pid);
		return NULL;
	}
	printk(KERN_INFO "%s: EXIT: res=%p pid=%d\n", __func__, res, current->pid);
	return res;
}
...
{
	printk(KERN_INFO "%s: ENTRY: ndd=%p, label_id=%p pid=%d\n", __func__, ndd, label_id, current->pid);
	...
	printk(KERN_INFO "%s: EXIT: allocated=%llu pid=%d\n", __func__, allocated, current->pid);
	return allocated;
}

static int count_dimms(struct device *dev, void *c)
{
	printk(KERN_INFO "%s: ENTRY: dev=%p, c=%p pid=%d\n", __func__, dev, c, current->pid);
	int *count = c;
...
{
	printk(KERN_INFO "%s: ENTRY: nvdimm_bus=%p, dimm_count=%d pid=%d\n", __func__, nvdimm_bus, dimm_count, current->pid);
	int count = 0;
	nd_synchronize();
	device_for_each_child(&nvdimm_bus->dev, &count, count_dimms);
	dev_dbg(&nvdimm_bus->dev, "count: %d\n", count);
	if (count != dimm_count) {
		printk(KERN_INFO "%s: EXIT: -ENXIO (count mismatch) pid=%d\n", __func__, current->pid);
		return -ENXIO;
	}
	printk(KERN_INFO "%s: EXIT: 0 pid=%d\n", __func__, current->pid);
	return 0;
