...
{
	printk(KERN_INFO "%s: ENTRY: dev=%p, env=%p pid=%d\n", __func__, dev, env, current->pid);
...
{
	printk(KERN_INFO "%s: ENTRY: nvdimm_bus=%p pid=%d\n", __func__, nvdimm_bus, current->pid);
...
{
	printk(KERN_INFO "%s: ENTRY: nd_region=%p, event=%d pid=%d\n", __func__, nd_region, event, current->pid);
	struct nvdimm_bus *nvdimm_bus = walk_to_nvdimm_bus(&nd_region->dev);

	if (!nvdimm_bus) {
		printk(KERN_INFO "%s: EXIT: nvdimm_bus=NULL pid=%d\n", __func__, current->pid);
		return;
	}
...
static int nvdimm_clear_badblocks_region(struct device *dev, void *data)
{
	...
	if (!is_memory(dev)) {
		printk(KERN_INFO "%s: EXIT: not memory pid=%d\n", __func__, current->pid);
		return 0;
	}
	...
	if (ctx->phys < nd_region->ndr_start ||
		...
		printk(KERN_INFO "%s: EXIT: not in region pid=%d\n", __func__, current->pid);
		return 0;
	}
...
{
	printk(KERN_INFO "%s: ENTRY: nvdimm_bus=%p, phys=%pa, cleared=%llu pid=%d\n", __func__, nvdimm_bus, &phys, cleared, current->pid);
...
long nvdimm_clear_poison(struct device *dev, phys_addr_t phys,
		unsigned int len)
{
	printk(KERN_INFO "%s: ENTRY: dev=%p, phys=%pa, len=%u pid=%d\n", __func__, dev, &phys, len, current->pid);
	struct nvdimm_bus *nvdimm_bus = walk_to_nvdimm_bus(dev);
...
struct nvdimm_bus *nvdimm_bus_register(struct device *parent,
		struct nvdimm_bus_descriptor *nd_desc)
{
	printk(KERN_INFO "%s: ENTRY: parent=%p, nd_desc=%p pid=%d\n", __func__, parent, nd_desc, current->pid);
	struct nvdimm_bus *nvdimm_bus;
	int rc;
	...
	if (rc) {
		...
		printk(KERN_INFO "%s: EXIT: device_add failed rc=%d pid=%d\n", __func__, rc, current->pid);
		goto err;
	}
	printk(KERN_INFO "%s: EXIT: nvdimm_bus=%p pid=%d\n", __func__, nvdimm_bus, current->pid);
	return nvdimm_bus;
 err:
...
void nvdimm_bus_unregister(struct nvdimm_bus *nvdimm_bus)
{
	printk(KERN_INFO "%s: ENTRY: nvdimm_bus=%p pid=%d\n", __func__, nvdimm_bus, current->pid);
	if (!nvdimm_bus) {
		printk(KERN_INFO "%s: EXIT: nvdimm_bus is NULL pid=%d\n", __func__, current->pid);
		return;
	}
...
{
	printk(KERN_INFO "%s: ENTRY: badrange_list=%p pid=%d\n", __func__, badrange_list, current->pid);
...
static int nd_bus_probe(struct device *dev)
{
	...
	printk(KERN_INFO "%s: EXIT: rc=0 pid=%d\n", __func__, current->pid);
	return 0;
}
...
static int nvdimm_bus_match(struct device *dev, const struct device_driver *drv)
{
	printk(KERN_INFO "%s: ENTRY: dev=%p, drv=%p pid=%d\n", __func__, dev, drv, current->pid);
...
{
	printk(KERN_INFO "%s: ENTRY: d=%p, cookie=%lu pid=%d\n", __func__, d, (unsigned long)cookie, current->pid);
	struct device *dev = d;
...
{
	printk(KERN_INFO "%s: ENTRY: dev=%p, sync=%d pid=%d\n", __func__, dev, sync, current->pid);
	if (!dev) {
		printk(KERN_INFO "%s: EXIT: dev is NULL pid=%d\n", __func__, current->pid);
		return;
	}
...
{
	printk(KERN_INFO "%s: ENTRY: dev=%p, mode=%d pid=%d\n", __func__, dev, mode, current->pid);
	bool killed;

	switch (mode) {
	case ND_ASYNC:
		if (!kill_device(dev)) {
			printk(KERN_INFO "%s: EXIT: not killed (ND_ASYNC) pid=%d\n", __func__, current->pid);
			return;
		}
		get_device(dev);
	...
	case ND_SYNC:
		...
		if (!killed) {
			printk(KERN_INFO "%s: EXIT: not killed (ND_SYNC) pid=%d\n", __func__, current->pid);
			return;
		}
...
int __nd_driver_register(struct nd_device_driver *nd_drv, struct module *owner,
		const char *mod_name)
{
	printk(KERN_INFO "%s: ENTRY: nd_drv=%p, owner=%p, mod_name=%s pid=%d\n", __func__, nd_drv, owner, mod_name, current->pid);
	struct device_driver *drv = &nd_drv->drv;

	if (!nd_drv->type) {
		pr_debug("driver type bitmask not set (%ps)\n",
				__builtin_return_address(0));
		printk(KERN_INFO "%s: EXIT: -EINVAL (no type) pid=%d\n", __func__, current->pid);
		return -EINVAL;
	}

	if (!nd_drv->probe) {
		pr_debug("%s ->probe() must be specified\n", mod_name);
		printk(KERN_INFO "%s: EXIT: -EINVAL (no probe) pid=%d\n", __func__, current->pid);
		return -EINVAL;
	}
...
{
	printk(KERN_INFO "%s: ENTRY: disk=%p pid=%d\n", __func__, disk, current->pid);
	struct device *dev = disk_to_dev(disk)->parent;
	struct nd_region *nd_region = to_nd_region(dev->parent);
	int disk_ro = get_disk_ro(disk);

	if (disk_ro == nd_region->ro) {
		printk(KERN_INFO "%s: EXIT: already correct pid=%d\n", __func__, current->pid);
		return;
	}
...
static int nvdimm_dev_to_target_node(struct device *dev)
{
	...
	if (!nd_region) {
		printk(KERN_INFO "%s: EXIT: NUMA_NO_NODE pid=%d\n", __func__, current->pid);
		return NUMA_NO_NODE;
	}
	printk(KERN_INFO "%s: EXIT: target_node=%d pid=%d\n", __func__, nd_region->target_node, current->pid);
		...
		int n)
{
	...
	if (!IS_ENABLED(CONFIG_NUMA)) {
		printk(KERN_INFO "%s: EXIT: 0 (NUMA not enabled) pid=%d\n", __func__, current->pid);
		return 0;
	}

	if (a == &dev_attr_target_node.attr &&
			nvdimm_dev_to_target_node(dev) == NUMA_NO_NODE) {
		printk(KERN_INFO "%s: EXIT: 0 (no target node) pid=%d\n", __func__, current->pid);
		return 0;
	}
...
int nvdimm_bus_create_ndctl(struct nvdimm_bus *nvdimm_bus)
{
	...
	if (rc) {
		dev_dbg(&nvdimm_bus->dev, "failed to register ndctl%d: %d\n",
				nvdimm_bus->id, rc);
		printk(KERN_INFO "%s: EXIT: dev_set_name failed rc=%d pid=%d\n", __func__, rc, current->pid);
		goto err;
	}
...
{
	printk(KERN_INFO "%s: ENTRY: nvdimm=%p, cmd=%d, desc=%p, idx=%d, buf=%p pid=%d\n", __func__, nvdimm, cmd, desc, idx, buf, current->pid);
	if (idx >= desc->in_num) {
		printk(KERN_INFO "%s: EXIT: UINT_MAX (idx >= in_num) pid=%d\n", __func__, current->pid);
		return UINT_MAX;
	}

	if (desc->in_sizes[idx] < UINT_MAX) {
		printk(KERN_INFO "%s: EXIT: size=%u pid=%d\n", __func__, desc->in_sizes[idx], current->pid);
		return desc->in_sizes[idx];
	}

	if (nvdimm && cmd == ND_CMD_SET_CONFIG_DATA && idx == 2) {
		struct nd_cmd_set_config_hdr *hdr = buf;
		printk(KERN_INFO "%s: EXIT: in_length=%u pid=%d\n", __func__, hdr->in_length, current->pid);
		...
		printk(KERN_INFO "%s: EXIT: nd_size_in=%u pid=%d\n", __func__, pkg->nd_size_in, current->pid);
		return pkg->nd_size_in;
	}

	printk(KERN_INFO "%s: EXIT: UINT_MAX (default) pid=%d\n", __func__, current->pid);
	return UINT_MAX;
}
...
{
	printk(KERN_INFO "%s: ENTRY: nvdimm=%p, cmd=%d, desc=%p, idx=%d, in_field=%p, out_field=%p, remainder=%lu pid=%d\n", __func__, nvdimm, cmd, desc, idx, in_field, out_field, remainder, current->pid);
	if (idx >= desc->out_num) {
		printk(KERN_INFO "%s: EXIT: UINT_MAX (idx >= out_num) pid=%d\n", __func__, current->pid);
		return UINT_MAX;
	}
	...
	if (nvdimm && cmd == ND_CMD_GET_CONFIG_DATA && idx == 1) {
		printk(KERN_INFO "%s: EXIT: in_field[1]=%u pid=%d\n", __func__, in_field[1], current->pid);
		return in_field[1];
	} else if (nvdimm && cmd == ND_CMD_VENDOR && idx == 2) {
		printk(KERN_INFO "%s: EXIT: out_field[1]=%u pid=%d\n", __func__, out_field[1], current->pid);
		return out_field[1];
	} else if (!nvdimm && cmd == ND_CMD_ARS_STATUS && idx == 2) {
		if (out_field[1] < 4) {
			printk(KERN_INFO "%s: EXIT: 0 (out_field[1] < 4) pid=%d\n", __func__, current->pid);
			return 0;
		}
		if (out_field[1] - 4 == remainder) {
			printk(KERN_INFO "%s: EXIT: remainder=%lu pid=%d\n", __func__, remainder, current->pid);
			return remainder;
		}
		printk(KERN_INFO "%s: EXIT: out_field[1]-8=%u pid=%d\n", __func__, out_field[1] - 8, current->pid);
		return out_field[1] - 8;
	} else if (cmd == ND_CMD_CALL) {
		struct nd_cmd_pkg *pkg = (struct nd_cmd_pkg *) in_field;
		printk(KERN_INFO "%s: EXIT: nd_size_out=%u pid=%d\n", __func__, pkg->nd_size_out, current->pid);
...
static int nd_pmem_forget_poison_check(struct device *dev, void *data)
{
	...
	if (nd_dax || !dev->driver) {
		printk(KERN_INFO "%s: EXIT: 0 (nd_dax or !dev->driver) pid=%d\n", __func__, current->pid);
		return 0;
	}
	...
	if (nd_btt || nd_pfn || nd_dax) {
		...
		if (!ndns) {
			printk(KERN_INFO "%s: EXIT: 0 (!ndns) pid=%d\n", __func__, current->pid);
			return 0;
		}
	} else
		ndns = to_ndns(dev);
	...
	if ((pstart >= start) && (pend <= end)) {
		printk(KERN_INFO "%s: EXIT: -EBUSY (overlap) pid=%d\n", __func__, current->pid);
		return -EBUSY;
	}
...
{
	printk(KERN_INFO "%s: ENTRY: nvdimm_bus=%p, nvdimm=%p, cmd=%u, data=%p pid=%d\n", __func__, nvdimm_bus, nvdimm, cmd, data, current->pid);
	...
	if (nd_desc->clear_to_send) {
		...
		if (rc) {
			printk(KERN_INFO "%s: EXIT: rc=%d (clear_to_send) pid=%d\n", __func__, rc, current->pid);
			return rc;
		}
	}
	...
	if (!nvdimm && cmd == ND_CMD_CLEAR_ERROR) {
		...
		printk(KERN_INFO "%s: EXIT: ret=%d (CLEAR_ERROR) pid=%d\n", __func__, ret, current->pid);
		return ret;
	}

	if (!nvdimm || cmd != ND_CMD_SET_CONFIG_DATA) {
		printk(KERN_INFO "%s: EXIT: 0 (not SET_CONFIG_DATA) pid=%d\n", __func__, current->pid);
		return 0;
	}

	wait_nvdimm_bus_probe_idle(&nvdimm_bus->dev);
	if (atomic_read(&nvdimm->busy)) {
		printk(KERN_INFO "%s: EXIT: -EBUSY (busy) pid=%d\n", __func__, current->pid);
		return -EBUSY;
	}
	printk(KERN_INFO "%s: EXIT: 0 pid=%d\n", __func__, current->pid);
	return 0;
...
{
	printk(KERN_INFO "%s: ENTRY: file=%p, cmd=0x%x, arg=%lx, mode=%d pid=%d\n", __func__, file, cmd, arg, mode, current->pid);
