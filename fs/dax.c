		...
		const struct iomap_ops *ops)
{
	...
	int ret;
		...
		printk(KERN_INFO "DAX_CORE: dax_iomap_rw ENTRY - len=%zu pos=%lld pid=%d\n",
	...
	if (iov_iter_rw(iter) == WRITE) {
		...
		printk(KERN_INFO "DAX_CORE: dax_iomap_rw STEP1 - WRITE operation pid=%d\n", current->pid);
	} else {
		lockdep_assert_held(&iomi.inode->i_rwsem);
		printk(KERN_INFO "DAX_CORE: dax_iomap_rw STEP1 - READ operation pid=%d\n", current->pid);
	}

	if (iocb->ki_flags & IOCB_NOWAIT)
		iomi.flags |= IOMAP_NOWAIT;

	printk(KERN_INFO "DAX_CORE: dax_iomap_rw STEP2 - Starting iomap iteration pid=%d\n", current->pid);
	...
	if (done > 0) {
		printk(KERN_INFO "DAX_CORE: dax_iomap_rw SUCCESS - done=%lld pid=%d\n", done, current->pid);
	} else if (ret < 0) {
		printk(KERN_INFO "DAX_CORE: dax_iomap_rw ERROR - ret=%d pid=%d\n", ret, current->pid);
	}
