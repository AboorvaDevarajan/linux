	...
	struct iov_iter		*to)
{
	...
	printk(KERN_INFO "DAX_XFS: xfs_file_dax_read ENTRY - size=%zu pid=%d\n", 
	...
	ret = xfs_ilock_iocb(iocb, XFS_IOLOCK_SHARED);
	if (ret) {
		printk(KERN_INFO "DAX_XFS: xfs_file_dax_read ERROR - Lock failed: %d pid=%d\n", ret, current->pid);
		return ret;
	}
	
	printk(KERN_INFO "DAX_XFS: xfs_file_dax_read STEP1 - Calling dax_iomap_rw pid=%d\n", current->pid);
	ret = dax_iomap_rw(iocb, to, &xfs_read_iomap_ops);
	xfs_iunlock(ip, XFS_IOLOCK_SHARED);

	if (ret >= 0) {
		printk(KERN_INFO "DAX_XFS: xfs_file_dax_read SUCCESS - bytes=%zd pid=%d\n", ret, current->pid);
	} else {
		printk(KERN_INFO "DAX_XFS: xfs_file_dax_read ERROR - ret=%zd pid=%d\n", ret, current->pid);
	}
	...
	struct iov_iter		*from)
{
	...
	loff_t			pos;
		...
		printk(KERN_INFO "DAX_XFS: xfs_file_dax_write ENTRY - size=%zu pid=%d\n",
		iov_iter_count(from), current->pid);

	ret = xfs_ilock_iocb(iocb, iolock);
	if (ret) {
		printk(KERN_INFO "DAX_XFS: xfs_file_dax_write ERROR - Lock failed: %d pid=%d\n", ret, current->pid);
		return ret;
	}
	ret = xfs_file_write_checks(iocb, from, &iolock, NULL);
	if (ret) {
		printk(KERN_INFO "DAX_XFS: xfs_file_dax_write ERROR - Write checks failed: %d pid=%d\n", ret, current->pid);
		goto out;
	}

	pos = iocb->ki_pos;

	printk(KERN_INFO "DAX_XFS: xfs_file_dax_write STEP1 - Calling dax_iomap_rw pid=%d\n", current->pid);
...
out:
	if (iolock)
		xfs_iunlock(ip, iolock);
	if (error) {
		printk(KERN_INFO "DAX_XFS: xfs_file_dax_write ERROR - Setfilesize failed: %d pid=%d\n", error, current->pid);
		return error;
	}

	if (ret > 0) {
		printk(KERN_INFO "DAX_XFS: xfs_file_dax_write SUCCESS - bytes=%zd pid=%d\n", ret, current->pid);
		XFS_STATS_ADD(ip->i_mount, xs_write_bytes, ret);

		/* Handle various SYNC-type writes */
		ret = generic_write_sync(iocb, ret);
	} else if (ret < 0) {
		printk(KERN_INFO "DAX_XFS: xfs_file_dax_write ERROR - Write failed: %zd pid=%d\n", ret, current->pid);
	}
	return ret;
}
